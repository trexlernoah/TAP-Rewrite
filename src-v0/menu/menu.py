...
# Function for loading "about" box
def show_about_info():
    messagebox.showinfo(
        title="About",
...
def set_subject_threshold():
    subject_threshold = Toplevel(window)
...
def profile_setup():
    ...
    if number_of_trials != None:
        ...
        profile_parameters(trial_num=number_of_trials)
    else:
        number_of_trials=0


def profile_parameters(trial_num):
    profile_parameters = Toplevel(window)
...
def init_main_window():
    ...
    create_new_menu.add_command(label="Experiment", command=lambda:[init_experiment("experiment"), profile_setup()])
    ...
    experiment_menu.add_command(label="Exit", command=window.destroy)

    # Threshold dropdown menu options
    threshold_menu.add_command(label="Set Subject Threshold", command=set_subject_threshold)
