...
class Drawer():
    ...
    def render_text(self, text, color=FG, delay=0):
        ...
        self.textbox.blit(text_block, text_block.get_rect(center = self.textbox.get_rect().center))
        pygame.display.flip()
    ...
    def draw_circles(self):
        ...
        self.surface.blit(low_text, (self.center_x - 230, (self.center_y + 10) + (self.center_y / 2)))

        high_text = self.font.render("High", 1, BLACK)
        self.surface.blit(high_text, (self.center_x + 220, (self.center_y + 10) + (self.center_y / 2)))

        # Initialize arc
        pygame.draw.circle(self.surface, BLACK, (self.center_x, self.center_y - 250), 100, width=5, draw_top_left=True, draw_top_right=True)
        pygame.draw.line(self.surface, BLACK, (self.center_x - 100, self.center_y - 250), (self.center_x + 100, self.center_y - 250), 5)
    ...
    def erase_meter(self, _key):
        ...
        offset = 275
        rect, startRad, endRad = self.clockwise_arc((self.center_x, self.center_y - 250), 95, 180, (180+(180/10 * key)))
        pygame.draw.circle(self.surface, WHITE, (self.center_x + offset, (self.center_y - 60) + (self.center_y / 2)), RADIUS, 39)
