...
def init_module():
    ...
    def event_generate(widget: "tkinter.Misc", sequence: str, data: Any = None):
        ...
        with generation_lock:
            ...
            widget.event_generate(sequence, data=str(index), when="tail")
    ...
    class ProcGenerator(Generator):
        ...
        def __call__(self, widget, func):
            event_generate(widget, virtual_event, self.EventData(self._handler, func))
    ...
    class FuncGenerator(Generator):
        ...
        def __call__(self, widget, func):
            if not (cell := return_cell()):
                ...
                cell.bell.clear()
                event_generate(
                    widget, virtual_event, self.EventData(self._handler, cell, func)
                )
    ...
    def make_proxy(generator: Generator, widget: "tkinter.Misc", obj: Any):
    ...
    v = vars()
    globals().update(
        {name: v[name] for name in ["bind", "event_generate", "ProxyBuilder"]}
    )
...
if __name__ == "__main__":
    ...
    def work():
        ...
        proxy.print(f"{name:16} foo_proxy.square({y!r}) returned {yy}")
        y = 11
        yy = foo_proxy.square(y)
        proxy.print(f"{name:16} foo_proxy.square({y!r}) returned {yy}")
    ...
    root.after(100, lambda: event_generate(root, "<<test>>", ["a", ["b", "c"]]))
    root.after(100, lambda: event_generate(root, "<<test>>", "'hi there'"))
    root.after(100, lambda: event_generate(root, "<<test>>", {"content": "hi there"}))
